...
- **Status:** DONE · **Depends on:** T-080 · **References:** DC §6, DC §7 (Extraction), Invariant 6
...
- ✓ 2026-06-14 — `extraction.agent.ts` (one `taskClass:"extraction"` call, no model named; outputSchema = derived ExtractionOutput JSON Schema; untrusted-output belt: entities dedupe on (name+type), a relationship is kept only when BOTH endpoints name a proposed entity — dangling edges never stage) + `extraction.repository.ts` (PRIVATE; stages `entity_proposals` + `relationship_proposals` in ONE tx, status `proposed`, NEVER the canonical `entities` graph — Invariant 6) + `extraction.handler.ts` (thin; actor-guarded before any spend/write; NO transitions — extracting→drafting is T-085's Flow). Migration `0005_swift_nitro.sql`; new prefixes `eprop_`/`rprop_` extend DOMAIN_MODEL §2 (flagged for owner blessing). Wired into the ai-orchestration module + pipeline worker (extraction stage live). Agent-contracts ExtractionInput/Output unchanged (T-061 stub sufficient — no version bump). typecheck + Biome clean; 6 tests green (agent fixtures + Testcontainers integration: proposals staged, dangling dropped, ledgered+correlated, malformed actor rejected before any write).

### T-082 · Verification agent (advisory)
- **Status:** DONE · **Depends on:** T-072 · **References:** DC §7 (Verification), Invariant 4
...
- ✓ 2026-06-14 — `verification.agent.ts` (one `taskClass:"reasoning"` call, no model named; reads the draft envelope GENERICALLY — block ids + text, NO content-schema import [Invariant 8]; outputSchema = derived VerificationOutput; untrusted belt: a flag aimed at a block id not in the draft is dropped, dupes collapse) + `verification.repository.ts` (PRIVATE; records `verification_flags` against (document, version); writes only flags) + `verification.handler.ts` (**depends on NO Editorial service — so it has no path to a status change: Invariant 4 by construction**; actor-guarded). Migration `0006_red_ogun.sql`; new prefix `vflag_` (flagged for owner blessing). Wired into module + worker. typecheck + Biome clean; 6 tests green — the acceptance proven LIVE in integration: flags recorded for the reviewer while the document **stays exactly `in_review`** (the agent cannot gate).

### T-083 · Atomization agent (post-publish)
- **Status:** DONE · **Depends on:** T-036 · **References:** DC §7 (Atomization)
...
- ✓ 2026-06-14 — `atomization.agent.ts` (one `taskClass:"reasoning"` call, no model named; reads the PUBLISHED envelope only [Invariant 3] — title + body via the shared `internal/envelope-text.ts` reader, no content-schema import [Invariant 8]; outputSchema = derived AtomizationOutput; untrusted belt drops blank thread posts/empty channels) + `atomization.repository.ts` (PRIVATE; one `social_variants` row per produced channel — x_thread/ig_caption/newsletter_blurb, payload jsonb) + `atomization.handler.ts` (post-publish, writes only variants — can't affect status) + `AiOrchestrationService.enqueueAtomization` (the trigger seam, idempotent on the publish id). Migration `0007_smooth_falcon.sql`; new prefix `svar_` (flagged). Wired into module + worker (all five stages now live — `STAGE_ARRIVES` retired). **Extracted `internal/envelope-text.ts`** (shared by Verification + Atomization — no re-roll). typecheck + Biome clean; 9 tests green (agent fixtures + Testcontainers integration: one variant per channel from the published envelope, ledgered+correlated; admin publish e2e unaffected). **The live publish→atomization wire lands in T-085's Flow** (where the e2e has real Redis) — the controller stays queue-free so the Redis-free publish e2e keeps passing.

### T-084 · Topic discovery (repeatable job)
- **Status:** DONE · **Depends on:** T-071 · **References:** DC §8, ID §6
...
- ✓ 2026-06-14 — `topic-discovery.agent.ts` (one `taskClass:"reasoning"` call, no model named; small output contract lives local — discovery isn't a DC §7 content agent; untrusted belt dedupes candidates on title, drops blanks) + `topic-discovery.handler.ts` (proposes via the agent, persists through **`EditorialService.proposeTopics`** — Invariant 7, never touches the topics table directly; actor-guarded; starts NO research — a human accepts a topic to begin, DOMAIN_MODEL §4) + `EditorialRepository.proposeTopics` (one-tx read-then-insert dedupe on case-insensitive title — repeatable-safe, no duplicate topics). The recurring job: `infra/queue/discovery.queue.ts` (a BullMQ **job scheduler**, `scheduleTopicDiscovery(queue, cron)` upserted idempotently) wired into the worker (default Mondays 09:00). No migration (topics existed). typecheck + Biome clean; tests green — agent fixtures + PG integration (topics land at `discovered`, dedupe holds, malformed actor rejected) + **Redis Testcontainers (the scheduler registers + the worker runs a fired job → "on schedule")**.

### T-085 · Wire the full Flow
- **Status:** DONE · **Depends on:** T-080, T-081, T-082, T-083, T-084 · **References:** DC §8, AR §9 & §12, Invariant 4
...
- ✓ 2026-06-14 — **`POST /admin/topics/:id/accept`** (`EditorialService.startTopic`: accept the topic → open the document at `discovered` → human `discovered→researching` → enqueue research with `correlationId = topic:<id>`). **`pipeline-advance.ts`** (`attachPipelineAdvance`, the `attachDraftingFailureAudit` pattern) turns the independent stage workers into one flow: on each completion it takes the SYSTEM-legal edge + enqueues the next — research→(researching→extracting)→extraction→(extracting→drafting)→drafting→[handler's drafting→in_review]→verification→**STOP** at in_review. Gate-safe: it only ever requests system edges (the T-034 machine rejects the rest) and a non-flow job (doc not in the expected state) raises IllegalTransition and is skipped — never forced. `AiOrchestrationService` got the per-stage enqueues (shared idempotent-jobid helper, correlation threaded) + a lazy-client error handler (a Redis outage can't crash the app). Publish triggers atomization (admin publish endpoint, fire-and-forget). Also fixed a latent bug: the drafting handler now strips `url` from sources before the strict Writing input (research citations carry one). typecheck + Biome clean; **full platform suite 151/151 green, 0 unhandled errors.** **The capstone `pipeline-flow.test.ts` (PG + Redis Testcontainers, real workers + advancement over the real gateway/scripted provider) proves the acceptance LIVE:** an accepted topic flows automatically to `in_review` (1 agent:writing version, staged proposals, advisory flags, audited path `researching→extracting→drafting→in_review`, ONE correlationId across research/extraction/writing/verification calls), **stops** (never auto-approves/publishes), then human approve+publish → atomization records all three channels. **★ PHASE 6 EXIT MET.**
...
### T-100 · Real auth
- **Status:** DONE · **Depends on:** T-038 · **References:** DC §12, ID §11, Invariant 4
...
- ✓ 2026-06-14 — `users` table (migration 0009; argon2id hash, role, unique email) + `modules/identity/`: **argon2id via `hash-wasm`** (pure-WASM — identical across dev/CI/containers, no native build; test cost profile keeps suites fast), **opaque server-side sessions in Redis** (`SessionStore`, lazy client → boot stays connection-free; httpOnly `arsyen_session` cookie carries only a 256-bit id; logout is a real revocation; expiry slides), `IdentityService` (createUser/authenticate — vague on failure, dummy-hash verify for missing users → no enumeration), `SessionGuard` (the `/admin` auth boundary — resolves the session and attaches the **server-derived** `human:<id>` actor) + `RolesGuard`/`@Roles` (admin-only user creation) + `@CurrentUser`. **The T-038 `x-arsyen-actor` header stub is GONE** — `@Actor()` now reads `request.user.actor`, so a client can no longer assert its own identity; downstream guards (state machine, publishing) are unchanged, and Invariant 4 is now LAYERED (only humans reach `/admin` at all, and the publish/workflow guards still reject non-human actors — `publishing.service.test`/`workflow.test`). `AuthController` `/admin/auth` (login/logout/me/users); all three `/admin` controllers behind `SessionGuard`. Filter maps UNAUTHENTICATED/INVALID_CREDENTIALS→401, FORBIDDEN_ROLE→403, EMAIL_ALREADY_EXISTS→409. Bootstrap login via `user:create` CLI (no public signup — invite-only, DC §12). Tests: password round-trip+salt, session lifecycle, service auth matrix (wrong pw/unknown/disabled/dup), and a full auth e2e (httpOnly cookie, 401 boundary, role gate, logout revocation, **actor derived from session proven via the audit trail**); the three `/admin` e2e suites re-driven through real login (the one `system`-only `drafting→in_review` step is invoked server-side, as the pipeline does). **Full platform suite: 46 files / 186 tests green.** Note: `drafting→in_review` stays `system`-only per DOMAIN_MODEL §4 (a human can't impersonate `system` anymore) — a manual-doc "submit for review" edge would be a §4 change, deferred to the founder.

### T-101 · OpenAPI from Zod DTOs
- **Status:** DONE · **Depends on:** T-037, T-038 · **References:** ID §10, DC §10
...
- ✓ 2026-06-14 — `@nestjs/swagger@11.4.4` added; `openapi/openapi.ts` builds the doc from the LIVE routes + the `createZodDto` classes (which carry `_OPENAPI_METADATA_FACTORY`) and runs `nestjs-zod`'s **`cleanupOpenApiDoc`** — `nestjs-zod@5` dropped `patchNestjsSwagger`, the v5 path is DTO-metadata + cleanup (recorded). `bootstrap.setupOpenApi` serves **Swagger UI `/v1/docs` + raw spec `/v1/openapi.json`** (the publish); both surfaces tagged (`v1` public / `admin` internal) with `@ApiTags`/`@ApiCookieAuth`/`@ApiBody({type: Dto})`/`@ApiParam`/`@ApiQuery` on every controller (the `/admin` bodies reference the Zod-derived component schemas; multipart upload via `@ApiConsumes`). New `openapi:generate` CLI (`cli/openapi-gen.ts`) dumps the artifact **infra-free** (a valid-shape synthetic env; nothing dialed) → CI can regenerate offline; the file is gitignored (derived, also served live). 4 contract tests (`openapi.test.ts`, container-free): doc metadata + both tags, **every `/v1` read route present and no write verb leaks** (the "matches the live surface" acceptance), the `/admin` surface present, and **request schemas derive from the Zod DTOs** (`CreateTopicDto.properties.title.type === "string"`, `required` excludes the optional `summary`, and `POST /admin/topics` body `$ref`s it). 20 paths generated. NOTE: additive ops/meta routes only — the published-envelope `/v1` *content* contract is unchanged, so no content-schema/agent-contracts CHANGELOG bump (DoD §6 N/A). typecheck + Biome clean.

### T-102 · Consumer onboarding + integration guide
- **Status:** DONE · **Depends on:** T-039, T-101 · **References:** DC §9, Invariants 1 & 2
...
- ✓ 2026-06-14 — The delivery side already existed (T-039: `consumers` table in migration `0001`, HMAC `signBody`/`verifySignature`, `OutboxDispatchService`) — T-102 added the **registration + secret management** surface and the **integration guide**. NO migration (table pre-existed). Producer: `ConsumersService` (publishing module — it owns the table, EG §3) `register`/`list`/`rotateSecret`/`setActive`; the shared HMAC secret is `randomBytes(32)` base64url, **returned exactly once** at register/rotate and **never echoed** after (the `ConsumerView` omits it; the dispatcher reads it to sign). Repo gained `listConsumers`/`findConsumerByName`/`findConsumerById`/`updateConsumerSecret`/`setConsumerActive`; typed `ConsumerNameTaken` (409) / `ConsumerNotFound` (404). **`AdminConsumersController` `/admin/consumers`** — register/list/rotate-secret/deactivate/activate, **admin-only** (`SessionGuard`+`RolesGuard`+`@Roles("admin")`, like user creation; registration is operator-driven — consumers are invite-only partners, DC §12), Swagger-annotated. Consumer side (reference, depends only on `content-schema`): `consumer-demo/src/webhook.ts` (`verifyWebhook` timing-safe + `WebhookReceiver` verify→dedupe-on-`x-arsyen-delivery`→parse) and **`consumer-demo/INTEGRATION.md`** — the self-contained guide (pull `/v1` ETag/`?schemaVersion=` + push signed webhooks; registration; signature verification with code; **§5a unknown-block degradation + §5b `schemaVersion` pinning** per Invariant 2; the runnable demo as the worked example). Tests: `consumers.service.test` (PG — secret-once+stored, list omits secret, name conflict, rotate, deactivate-excludes-from-delivery, and a **register→deliver→verify** loop proving a registered consumer verifies a real signed delivery), `admin-consumers.controller` e2e (401 unauth / 403 non-admin editor / 400 bad URL / register+list-without-secret+rotate+deactivate+404), and `consumer-demo/webhook.test` (valid/tampered/forged + dedupe). **Bug found & fixed (full-suite parallelism exposed it):** the T-103 Bull Board mounter tore its shared Redis client down mid-setup → an unhandled "Connection is closed" rejection; fixed by draining `waitUntilReady` (allSettled) before closing the queues and graceful-QUITing the client. **Full platform suite green & stable across 2 runs: 52 files / 211 tests, 0 unhandled errors; consumer-demo 10/10; all 6 packages build.** Decision recorded: the integration guide lives in `apps/consumer-demo/` (next to the runnable reference), not the hub — it's external API docs, not internal architecture context.

### T-103 · Observability hardening
- **Status:** DONE · **Depends on:** T-085 · **References:** DC §13, ID §13
...
- ✓ 2026-06-14 — Added `nestjs-pino@4.6.1`/`pino@10`/`pino-http@11` + `@sentry/node@10`. **Logging** (`infra/observability/logging.ts`): global `LoggerModule` whose `genReqId` adopts an inbound `x-correlation-id`/`x-request-id` (else mints a `corr_` TypeID) and **echoes it on the response**, surfaced in logs as `correlationId`; pino is the app logger (`useLogger`, `bufferLogs`); auth/cookie headers redacted (EG §7); silent under test/Vitest (header still set). The same id already rides `PipelineJobData` — the orchestration service now logs `stage enqueued` and each worker logs `stage completed`/`failed` with it, so one id threads **topic → research → extraction → drafting → verification → publish → atomization** (DC §13); workers + `admin`/`ai-costs`/`model-gateway` console.* calls replaced with structured pino. **Sentry** (`infra/observability/sentry.ts`, init-guarded → no DSN = no-op, offline CI): initialized in `bootstrap` + all three worker entries; the exception filter reports **only the unhandled 500 path** (tagged with correlationId), never mapped/HTTP errors; worker `failed` handlers + the daily-spend alert (the `ai-costs` TODO) capture too. **Bull Board** mounted IN-APP at `/internal/queues` behind **HTTP Basic auth** (`BULL_BOARD_USER`/`BULL_BOARD_PASS`, constant-time compare, 401+`WWW-Authenticate` before the router touches Redis) via `BullBoardModule` — included only when creds are set (fail safe); the standalone unauthenticated `board.dev.ts` + `queues:board` script were **retired** (superseded, as their own comments anticipated). **Deviation (recorded): also gated `GET /internal/ai-costs` behind `SessionGuard`** — that internal spend dashboard was unauthenticated and its own comment scheduled its auth for T-103 (DC §10/§13); `ModelGatewayModule` now imports `IdentityModule` (a leaf — no cycle), and its e2e was reworked to seed+login (+ a 401-without-session assertion). Tests: filter unit (reports unhandled, ignores mapped/HTTP, default reporter is a safe no-op), correlation e2e (id stamped + caller id adopted), board-disabled fail-safe (404) + internal-route 401, and a Redis-Testcontainers board auth e2e (401 no/bad creds, board served with creds). **Full platform suite: 50 files / 201 tests green.** Deferred per ID §13: OpenTelemetry (trigger-based).

### T-104 · Deploy
- **Status:** DONE · **Depends on:** T-003, T-100 · **References:** ID §14, DC §2
...
- ✓ 2026-06-14 — **`deploy/Dockerfile`** — ONE platform image (ID §14), Debian bookworm (sharp's prebuilt binaries are glibc), multi-stage (builder installs+`pnpm build`; runtime copies the built tree so pnpm's relative workspace symlinks + root `infra/migrations` + `config/` resolve in place). The command selects the process: API `node dist/main.js` (default CMD, `HEALTHCHECK` → `/health`), migrate `node dist/cli/migrate.js`, workers `node dist/workers/{pipeline,outbox,assets}-worker.dev.js`. **`src/cli/migrate.ts`** — production migrator using `drizzle-orm/postgres-js/migrator` (NOT drizzle-kit, a devDep absent at runtime): `CREATE EXTENSION IF NOT EXISTS vector` then applies `infra/migrations`, so a fresh MANAGED PG (no `init-pgvector.sql`) gets the vector-typed migrations (T-070/T-090); cwd-independent folder resolve + `MIGRATIONS_DIR` override (`db:migrate:deploy` script). **`deploy/docker-compose.prod.yml`** — the self-host VPS fork (the only file the hosting fork changes): pgvector + Redis(noeviction) + MinIO(R2 stand-in) + one-shot migrate (`service_completed_successfully` gate) → api + the three workers. **`.github/workflows/deploy.yml`** — on `main`: build + push the image to **GHCR** (works out of the box via `GITHUB_TOKEN`+`packages:write`, gha cache) + an inert host-deploy hook (gated on `DEPLOY_HOOK_URL`, mirroring the T-021 publish pattern). `.dockerignore` (root) + **`deploy/DEPLOY.md`** (managed path A: Railway/Render/Fly + Neon/Upstash/R2, with the migrate-then-roll release order; self-host path B; prod env table). **studio image deferred** — its build is still a stub (lands with the Next.js app). **Acceptance verified LIVE against the built image:** `docker build` → migrate runner applied migrations against PG → the API container booted in production mode (structured pino, `/health` 200) → a seeded published document was **reachable via `GET /v1/documents/{slug}` (200, self-contained envelope + ETag + `X-Schema-Version` + `x-correlation-id`)**, with `/v1/documents` and `/v1/openapi.json` (24 paths) serving. typecheck + Biome clean; full suite 52 files / 211 tests green; 6/6 build. **★ PHASE 8 EXIT MET — T-100…T-104 complete. The editorial pipeline is feature-complete; Phase 9+ is trigger-based only.**
